...
void find(List &list, int data) {
	...
	if (is_Empty(list)) {
		...
		if (cur != nullptr) std::cout << "»скомый элемент найден под индексом: " << count << std::endl;
		else std::cout << "Ёлемент не найден" << std::endl;
	}
}
...
void rfind(List &list, int data) {
	...
	if (is_Empty(list)) {
		...
		if (cur != nullptr) std::cout << "»скомый элемент найден под индексом: " << list.count - count << std::endl;
		else std::cout << "Ёлемент не найден" << std::endl;
	}
}
...
void push(List &list) {
	...
	if (is_Empty(list)) {
		...
		std::cout << "¬ведите элемент, относительно которого хотите выполнить добавление" << std::endl;
		std::cin >> data;
		std::cout << "¬ведите способ добавлени€:\n0. ¬ставить перед элементом\n1. ¬ставить после элемента" << std::endl;
		command = failure();
		if (command == 0) {
